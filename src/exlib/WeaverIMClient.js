(function (global, undefined) {
  ...
  function RongIMClient(domain, severip, serverport, resource) {
    ...
    this.connect = function (_id, _udid, _connectionListener) {
      ...
      weaverBridge.connect(
        _id,
        _udid,
        RongIMClient.connect.token,
        _connectionListener,
        RongIMClient.connect.callback
      );
  ...
  (function (w) {
    ...
    WeaverBridge = function (domain, severip, serverport, resource) {
      ...
      //xmpp消息中UDID必须使用小写！！！
      // 3379|1-mdvckbqdu-5g-vspxvlq@iz23t7ry645z
      // 3379|1-mdvckbqdu-5g-vspxvlq
      //
      // 3379
      // 3379|1-mdvckbqdu-5g-vspxvlq
      ...
      this.connect = function (_id, _udid, _p, _connectionListener, _callback) {
        ...
        stropheObj.connect(
          self.getXMPPUserIMId(_id) + '@' + self.serverIp + '/' + self.resource,
          _p, // token
          onConnectionStatusChanged,
          60,
          1,
          self.resource,
          self.getXMPPUserIMId(_id)
        );
          ...
          function wapperMessage(data) {
            ...
            if (bodyElems.length > 0 && (type == "chat" || type == "normal")) {
              ...
              if (!isDuplicateMessage(id)) {  //重复消息不处理
                var body = bodyElems[ 0 ].hasOwnProperty('innerHTML')
                  ? bodyElems[ 0 ].innerHTML
                  : bodyElems[ 0 ].textContent;
      ...
      this.sendPresenceMessage = function (_from, _priority) {
        var data = $pres({ id: getUUID(), from: _from })
          .c('priority', _priority).tree();
        self._execSend(data);
      };
      ...
      this.getHistoryMessage = function (targetType, fromUserId, targetId, lastTime, pageSize, callback) {
        ...
        function wapperGetHistory(data) {
          ...
          var innerHTML = queryElems[ 0 ].hasOwnProperty('innerHTML')
            ? queryElems[ 0 ].innerHTML
            : queryElems[ 0 ].textContent;
